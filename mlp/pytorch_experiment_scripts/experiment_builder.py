...
class ExperimentBuilder(nn.Module):
    def __init__(self, network_model, experiment_name, num_epochs, train_data, val_data,
        ...
        '''
        The default value of the current learning rate is set to 0.001.
        If you need to switch the learning rate to 0.01, switch the code below to: 
    ...
    def plot_grad_flow(self, named_parameters):
        ...
        for name,parms in named_parameters:
            ...
            if name.split('.')[-1]=='weight' and name.split('.')[-2].find('conv')>=0:
                layers.append(name.split('.')[1]+'.'+name.split('.')[-2])
                grad = torch.mean(torch.abs(parms.grad)).item()
